import { db, storage, authReady, isSignedIn } from "./firebase.js";
 ...
 * - text: answerText + explainText (optional) (also keep legacy answer for compatibility)
 * - choice: choices[], correctIndex, explain (optional)
 */

// -----------------------------
// Error helpers (สำคัญ: กัน Uncaught (in promise))
// -----------------------------
function handleFsError(e, context = "") {
  const code = e?.code || "";
  const msg = e?.message || String(e);

  console.error(`[Firestore:${context}]`, code, msg, e);

  if (code === "permission-denied") {
    toast("Firestore: สิทธิ์ไม่พอ (permission-denied) — ตรวจ Rules/Path/App Check");
    return;
  }
  toast(msg);
}

async function safeFirestore(fn, context) {
  await authReady; // รอ auth state พร้อม ลดเคสยิงก่อนระบบพร้อม
  try {
    return await fn();
  } catch (e) {
    handleFsError(e, context);
    return null;
  }
}

function requireSignedInForWrite() {
  if (!isSignedIn()) {
    toast("ต้องเข้าสู่ระบบก่อนจึงจะเพิ่ม/แก้ไข/ลบได้");
    return false;
  }
  return true;
}
...
export async function renderAdminLessons() {
  ...
  const courses = await safeFirestore(() => getAllCourses(), "getAllCourses");
  if (!courses) {
    panel.innerHTML = `<div class="muted">โหลดคอร์สไม่ได้ (สิทธิ์ไม่พอ/Rules)</div>`;
    return;
  }
  ...
  qs("#alAddLesson").addEventListener("click", async () => {
    ...
    if (!requireSignedInForWrite()) return;
    ...
    const ok = await safeFirestore(
      () =>
        addDoc(collection(db, COURSES, courseId, "lessons"), {
          title,
          order,
          createdAt: new Date(),
        }),
  ...
  await safeFirestore(() => renderLessonsList(window.__adminLessonsCourseId), "renderLessonsList");
}
...
async function renderLessonsList(courseId) {
  ...
  const lessons = await safeFirestore(() => getLessons(courseId), "getLessons");
  if (!lessons) {
    list.innerHTML = `<div class="muted">โหลดบทเรียนไม่ได้ (สิทธิ์ไม่พอ/Rules)</div>`;
    return;
  }
  ...
  list.querySelectorAll("[data-lesson-save]").forEach((btn) => {
    btn.addEventListener("click", async () => {
      ...
      const ok = await safeFirestore(
        () => updateDoc(doc(db, COURSES, courseId, "lessons", lessonId), { title, order }),
        "updateLesson"
      );
      if (!ok) return;
  ...
  list.querySelectorAll("[data-lesson-del]").forEach((btn) => {
    btn.addEventListener("click", async () => {
      ...
      const quizzes = await safeFirestore(() => getQuizzes(courseId, lessonId), "getQuizzesBeforeDeleteLesson");
      if (!quizzes) return;

      for (const q of quizzes) {
        const ok = await safeFirestore(
          () => deleteDoc(doc(db, COURSES, courseId, "lessons", lessonId, "quizzes", q.id)),
          "deleteQuizInLessonDelete"
        );
        if (!ok) return;
      }

      const ok2 = await safeFirestore(
        () => deleteDoc(doc(db, COURSES, courseId, "lessons", lessonId)),
        "deleteLesson"
      );
      if (!ok2) return;
  ...
  for (const lesson of lessons) {
    await safeFirestore(() => renderQuizList(courseId, lesson.id), "renderQuizList");
    ...
    form.querySelector(`[data-quiz-save="${lesson.id}"]`).addEventListener("click", async () => {
      ...
      if (payload.type === "text") {
        ...
        const explainText = form.querySelector(`[data-new-explain-text="${lesson.id}"]`)?.value.trim() || "";
        payload.explainText = explainText;
      } else {
        const choices = readChoicesFromForm(form, lesson.id, "new");
        const correctIndex = Number(form.querySelector(`[data-new-correct="${lesson.id}"]`).value || 0);
      ...
      const ok = await safeFirestore(
        () => addDoc(collection(db, COURSES, courseId, "lessons", lesson.id, "quizzes"), payload),
        "addQuiz"
      );
      if (!ok) return;
  ...
  return `
    <div id="quizAddForm-${lessonId}" class="card" style="display:none;margin-top:10px;background:rgba(255,255,255,.04);border:1px solid rgba(255,255,255,.10)">
      ...
      <div data-new-box-text="${lessonId}">
        ...
        <label class="label" style="margin-top:10px">คำอธิบายเฉลย (พิมพ์ตอบ) <span class="muted">(ไม่บังคับ)</span></label>
        <textarea class="input" rows="3" data-new-explain-text="${lessonId}" placeholder="อธิบายเหตุผล/บริบท/ไวยากรณ์ เพื่อแสดงตอนผู้เรียนกดดูเฉลย"></textarea>
      </div>
...
async function renderQuizList(courseId, lessonId) {
  ...
  const quizzes = await safeFirestore(() => getQuizzes(courseId, lessonId), "getQuizzes");
  if (!quizzes) {
    box.innerHTML = `<div class="muted">โหลดแบบฝึกหัดไม่ได้ (สิทธิ์ไม่พอ/Rules)</div>`;
    return;
  }
  ...
  box.querySelectorAll("[data-q-save]").forEach((btn) => {
    btn.addEventListener("click", async () => {
      ...
      let updatePayload = { question, content, type, order, imageUrl: "" };
      ...
      if (type === "text") {
        ...
        updatePayload.answer = answerText;
        updatePayload.choices = [];
        updatePayload.correctIndex = 0;
        updatePayload.explain = "";
        const explainText = qs(`[data-q-explain-text="${key}"]`)?.value.trim() || "";
        updatePayload.explainText = explainText;
      } else {
        ...
        updatePayload.answer = updatePayload.answerText;
      }
      ...
      const ok = await safeFirestore(
        () => updateDoc(doc(db, COURSES, courseId, "lessons", lId, "quizzes", qId), updatePayload),
        "updateQuiz"
      );
      if (!ok) return;
  ...
  box.querySelectorAll("[data-q-del]").forEach((btn) => {
    btn.addEventListener("click", async () => {
      ...
      const ok = await safeFirestore(
        () => deleteDoc(doc(db, COURSES, courseId, "lessons", lId, "quizzes", qId)),
        "deleteQuiz"
      );
      if (!ok) return;
  ...
  return `
    <div class="card" style="margin:10px 0;background:rgba(255,255,255,.03);border:1px solid rgba(255,255,255,.10)">
      ...
      <div data-q-box-text="${key}">
        ...
        <label class="label" style="margin-top:10px">คำอธิบายเฉลย (พิมพ์ตอบ) <span class="muted">(ไม่บังคับ)</span></label>
        <textarea class="input" rows="3" data-q-explain-text="${key}">${escapeHtml(q.explainText || "")}</textarea>
      </div>
...
function readChoicesFromForm(rootEl, lessonOrKey, mode) {
  const sel =
    mode === "new" ? `[data-new-choice="${lessonOrKey}"]` : `[data-q-choice="${lessonOrKey}"]`;
...
// =====================================================
// ✅ NEW: Admin Dashboard / Students / Docs (GitHub Raw)
// =====================================================


function ensureAdmin(panelName = "แอดมิน") {
  if (!isSignedIn()) {
    toast(`ต้องล็อกอิน${panelName}ก่อน`);
    return false;
  }
  return true;
}
...
export async function renderAdminDashboard() {
  if (!ensureAdmin("แอดมิน")) return;

  await authReady;

  // status box
  const box = qs("#adminStatusBox");
  if (box) box.innerHTML = `<div class="muted">กำลังโหลด...</div>`;

  try {
    // quick counts
    const studentsSnap = await getDocs(query(collection(db, "students"), limit(5000)));
    const coursesSnap = await getDocs(collection(db, "courses"));
    let live = 0;
    studentsSnap.forEach((d) => {
      const x = d.data() || {};
      if (x.liveJoined) live += 1;
    });

    if (box) {
      box.innerHTML = `
        <div>ผู้เรียนทั้งหมด: <b>${studentsSnap.size}</b></div>
        <div>เข้าร่วมเรียนสด: <b>${live}</b></div>
        <div>คอร์สทั้งหมด: <b>${coursesSnap.size}</b></div>
      `;
    }
  } catch (e) {
    if (box) box.innerHTML = `<div class="muted">โหลดสถานะไม่สำเร็จ: ${escapeHtml(e?.message || "")}</div>`;
  }

  // chart
  try {
    const mod = await import("./charts.js");
    if (typeof mod.renderAdminDashboardChart === "function") {
      await mod.renderAdminDashboardChart();
    }
  } catch (e) {
    console.warn("admin chart load failed:", e?.code, e?.message);
  }
}
...
export async function renderAdminStudents() {
  ...
  const panel = qs("#adminStudentsPanel");
  if (!panel) return;

  panel.innerHTML = `<div class="muted">กำลังโหลดรายชื่อ...</div>`;

  try {
    const snap = await getDocs(query(collection(db, "students"), orderBy("joinedAt", "desc")));
    const rows = [];
    ...
    panel.innerHTML = `
      <div style="display:flex;justify-content:space-between;gap:10px;align-items:flex-end;flex-wrap:wrap">
        <div>
          <div style="font-weight:700">ผู้เรียนทั้งหมด (${rows.length})</div>
          <div class="small muted">ลบ/รีเซ็ตสถานะเรียนสดได้</div>
        </div>
        <button class="btn btn-secondary" id="btnRefreshStudents">รีเฟรช</button>
      </div>
      ...
      <div style="overflow:auto;margin-top:12px">
        <table class="table">
          ...
          <tbody>
            ${rows
              .map((s) => {
                const joined = s.liveJoined ? "✅" : "—";
                const t = s.liveJoinedAt?.toDate ? s.liveJoinedAt.toDate() : s.liveJoinedAt;
                const tStr = t ? new Date(t).toLocaleString() : "";
                return `
                  <tr>
                    <td>${escapeHtml(s.fullname || "")}</td>
                    <td>${escapeHtml(s.courseId || "")}</td>
                    <td>${joined}</td>
                    <td class="small muted">${escapeHtml(tStr)}</td>
                    <td>
                      <button class="btn btn-ghost" data-reset-live="${s.id}">รีเซ็ตเรียนสด</button>
                      <button class="btn btn-danger" data-del-student="${s.id}">ลบ</button>
                    </td>
                  </tr>
                `;
              })
    ...
    qs("#btnRefreshStudents")?.addEventListener("click", () => renderAdminStudents());
    ...
    panel.querySelectorAll("[data-reset-live]").forEach((btn) => {
      btn.addEventListener("click", async () => {
        const id = btn.dataset.resetLive;
        await updateDoc(doc(db, "students", id), { liveJoined: false, liveJoinedAt: null });
        toast("รีเซ็ตแล้ว");
        await renderAdminStudents();
      });
    });
  } catch (e) {
    panel.innerHTML = `<div class="muted">โหลดรายชื่อไม่สำเร็จ: ${escapeHtml(e?.message || "")}</div>`;
  }
}
...
export async function renderAdminDocs() {
  ...
  const panel = qs("#adminDocsPanel");
  if (!panel) return;

  // courses for dropdown (robust: avoid orderBy/index issues)
  let courses = [];
  try {
    const cSnap = await getDocs(collection(db, "courses"));
    cSnap.forEach((d) => courses.push({ id: d.id, ...(d.data() || {}) }));

    // sort by order -> name -> id
    courses.sort((a, b) => {
    ...
    console.warn("load courses failed:", e?.code, e?.message);
  }

  panel.innerHTML = `
    <div class="grid-2">
      <div class="card" style="background:rgba(255,255,255,.04);border:1px solid rgba(255,255,255,.10)">
        <div style="font-weight:700">เพิ่มเอกสาร (ใช้ GitHub Raw URL)</div>

        <label class="label">คอร์ส</label>
        <select id="docCourseId" class="input">
          <option value="" selected disabled>— เลือกคอร์ส —</option>
          ${
            courses.length
              ? courses
                  .map(
                    (c) =>
                      `<option value="${escapeHtmlAttr(c.id)}">${escapeHtml(
                        c.name || c.id
                      )}</option>`
                  )
                  .join("")
              : `<option value="" disabled>(ยังไม่มีคอร์สในระบบ)</option>`
          }
        </select>

        <label class="label">ชื่อเอกสาร</label>
        <input id="docTitle" class="input" placeholder="เช่น Slide Day 1" />

        <label class="label">ลิงก์ไฟล์ (GitHub Raw)</label>
        <input id="docUrl" class="input" placeholder="https://raw.githubusercontent.com/.../file.pdf" />

        <div class="small muted" style="margin-top:6px">
          แนะนำ: ใช้ raw.githubusercontent.com หรือ GitHub ?raw=1 ที่เปิดได้ตรง
        </div>

        <div style="margin-top:10px;display:flex;justify-content:flex-end">
          <button id="btnAddDoc" class="btn btn-primary">+ เพิ่มเอกสาร</button>
        </div>
      </div>

      <div class="card" style="background:rgba(255,255,255,.04);border:1px solid rgba(255,255,255,.10)">
        <div style="font-weight:700">รายการเอกสาร</div>
        <div id="docList" class="muted" style="margin-top:10px">เลือกคอร์สเพื่อดูรายการเอกสาร</div>
      </div>
    </div>
  `;

  const courseSel = qs("#docCourseId");

  // เลือกคอร์สแรกให้อัตโนมัติ
  if (courseSel && courses.length) {
    courseSel.value = courses[0].id;
  }

  async function loadList() {
    const list = qs("#docList");
    const courseId = qs("#docCourseId")?.value || "";
    if (!list) return;

    if (!courseId) {
      list.innerHTML = `<div class="muted">เลือกคอร์สเพื่อดูรายการเอกสาร</div>`;
      return;
    }

    // NOTE: query+where อาจต้องมี index หากมี orderBy เพิ่ม ดังนั้นไม่ใส่ orderBy
    const snap = await getDocs(
      query(collection(db, "documents"), where("courseId", "==", courseId))
    );

    const items = [];
    snap.forEach((d) => items.push({ id: d.id, ...(d.data() || {}) }));

    // sort by createdAt desc (ทำในฝั่ง JS กันปัญหา index)
    items.sort((a, b) => {
          ...
          .map(
            (x) => `
              <div class="card" style="margin:10px 0;background:rgba(255,255,255,.03);border:1px solid rgba(255,255,255,.10)">
                ...
                <div class="small muted" style="word-break:break-all">${escapeHtml(x.fileUrl || "")}</div>
                <div style="margin-top:8px;display:flex;gap:8px;flex-wrap:wrap">
                  <a class="btn btn-secondary" href="${escapeHtmlAttr(
                    x.fileUrl || "#"
                  )}" target="_blank" rel="noreferrer">เปิดลิงก์</a>
                  <button class="btn btn-danger" data-del-doc="${escapeHtmlAttr(
                    x.id
                  )}">ลบ</button>
                </div>
              </div>
  ...
  courseSel?.addEventListener("change", loadList);
  ...
  qs("#btnAddDoc")?.addEventListener("click", async () => {
    ...
    if (!courseId) return toast("กรุณาเลือกคอร์ส");
    if (!title) return toast("กรุณาใส่ชื่อเอกสาร");
    if (!fileUrl) return toast("กรุณาใส่ลิงก์ GitHub Raw");
