...
export async function renderQuizzes(){
  ...
  // ✅ restore attempts/solved/revealed
...
function renderQuiz(panel){
  ...
  if (quizState.solved[q.id]) {
    ...
    if (type === "choice") {
      ...
      answer =
        ...
        (q.explain && String(q.explain).trim()) ||
        "(ไม่มีเฉลย)";
      explainText = (q.explainText && String(q.explainText).trim()) || "";
    }
  ...
  // ข้อสุดท้าย + ผิดครบ 2 ครั้ง -> ต้องกดเฉลยก่อน
  if (isLast && !canGoNext && tries >= 2 && !isSolved && !isRevealed) {
    msgEl.innerHTML = '❗ตอบผิดครบ 2 ครั้ง กรุณากดปุ่ม <b>ดูเฉลยคำตอบ</b> ก่อน';
  }

  // ✅ แก้ SyntaxError: ใช้ backtick
  if (isLast && canGoNext) {
    const doneLine = `
      <div style="margin-top:6px">
        🎉 ทำครบทุกข้อแล้ว กด <b>เสร็จสิ้น</b> เพื่อกลับไปหน้าเมนูบทเรียน
      </div>
    `;
    msgEl.innerHTML = (msgEl.innerHTML || "") + doneLine;
    msgEl.style.color = "rgba(241,210,138,.95)";
  }
  ...
  if(audioEl && btnToggle && btnDown && btnUp && rateEl){
    ...
    const renderRate = ()=>{ rateEl.textContent = `${rate.toFixed(1)}x`; };
    const syncBtn = ()=>{ btnToggle.textContent = audioEl.paused ? "▶️ เล่นเสียง" : "⏸️ หยุดเสียง"; };
    ...
    btnToggle.addEventListener("click", async ()=>{
      try{
        ...
        console.error(err);
        toast("เล่นเสียงไม่ได้: ตรวจสอบว่าเป็นลิงก์ .mp3 แบบ raw และเปิดผ่าน https");
      }
    });
