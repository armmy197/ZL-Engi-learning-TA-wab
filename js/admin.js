...
export async function renderAdminDocs() {
  ...
  let courses = [];
  try {
    const cSnap = await getDocs(query(collection(db, "courses"), orderBy("order")));
    ...
    <div class="grid-2">
      <div class="card" style="background:rgba(255,255,255,.04);border:1px solid rgba(255,255,255,.10)">
        ...
        <select id="docCourseId" class="input">
          ${courses.map((c) => `<option value="${escapeHtml(c.id)}">${escapeHtml(c.name || c.id)}</option>`).join("")}
        </select>

        <div class="card" style="margin-top:10px;background:rgba(255,255,255,.03);border:1px solid rgba(255,255,255,.10)">
          <div class="small muted">สิทธิ์เอกสารเรียน (เปิด/ปิด)</div>
          <div style="margin-top:6px;display:flex;align-items:center;justify-content:space-between;gap:10px;flex-wrap:wrap">
            <div id="docAccessState" style="font-weight:700">—</div>
            <button id="btnToggleDocsOpen" class="btn btn-secondary">สลับสถานะ</button>
          </div>
          <div class="small muted" style="margin-top:6px">ผู้เรียนจะดาวน์โหลดได้เมื่อ (1) เข้าร่วมเรียนสดแล้ว และ (2) แอดมินเปิดสิทธิ์เอกสารของคอร์สนี้</div>
        </div>
      ...
      <div class="card" style="background:rgba(255,255,255,.04);border:1px solid rgba(255,255,255,.10)">
        ...
        <div id="docList" class="muted" style="margin-top:10px">กำลังโหลด...</div>
      </div>
    </div>
  `;


  async function refreshDocsAccessUI() {
    const courseId = qs("#docCourseId")?.value || "";
    const stateEl = qs("#docAccessState");
    if (!courseId || !stateEl) return;
    try {
      const cs = await getDoc(doc(db, "courses", courseId));
      const c = cs.exists() ? cs.data() : {};
      const open = !!c.docsOpen;
      stateEl.textContent = open ? "✅ เปิดสิทธิ์เอกสาร" : "⛔ ปิดสิทธิ์เอกสาร";
    } catch (e) {
      stateEl.textContent = "—";
    }
  }

  qs("#docCourseId")?.addEventListener("change", async () => {
    await refreshDocsAccessUI();
    await loadList();
  });

  qs("#btnToggleDocsOpen")?.addEventListener("click", async () => {
    const courseId = qs("#docCourseId")?.value || "";
    if (!courseId) return toast("กรุณาเลือกคอร์ส");
    const cs = await getDoc(doc(db, "courses", courseId));
    const c = cs.exists() ? cs.data() : {};
    const next = !c.docsOpen;
    await updateDoc(doc(db, "courses", courseId), { docsOpen: next });
    toast(next ? "เปิดสิทธิ์เอกสารแล้ว" : "ปิดสิทธิ์เอกสารแล้ว");
    await refreshDocsAccessUI();
  });
  ...
  async function loadList() {
    ...
    const snap = await getDocs(query(collection(db, "documents"), where("courseId","==", (qs("#docCourseId")?.value || ""))));
    const items = [];
    ...
    list.innerHTML = items.length
      ? items
        ...
        <div class="card" style="margin:10px 0;background:rgba(255,255,255,.03);border:1px solid rgba(255,255,255,.10)">
          ...
          <div class="small muted">courseId: ${escapeHtml(x.courseId || "")}</div>
          <div class="small muted" style="word-break:break-all">${escapeHtml(x.fileUrl || "")}</div>
          <div style="margin-top:8px;display:flex;gap:8px;flex-wrap:wrap">
            <a class="btn btn-secondary" href="${escapeHtml(x.fileUrl || "#")}" target="_blank" rel="noreferrer">เปิดลิงก์</a>
            <button class="btn btn-danger" data-del-doc="${x.id}">ลบ</button>
          </div>
        </div>
      `
          )
